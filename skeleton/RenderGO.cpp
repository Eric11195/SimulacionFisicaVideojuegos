...
namespace render_go {
	void render3D(RenderItem* r_it)
	{
		auto objTransform = r_it->transform;
		if (!objTransform)
		{
			auto actor = r_it->actor;
			if (actor)
			{
				render_shape(*r_it->shape, actor->getGlobalPose(), r_it->color);
				return;
			}
		}
		render_shape(*r_it->shape, objTransform ? *objTransform : physx::PxTransform(PxIdentity), r_it->color);
	}
...
#define MAX_NUM_MESH_VEC3S  1024
	...
	void render_geometry(const PxGeometryHolder& h, bool wireframe)
	{
